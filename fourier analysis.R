...
dat.al.t2$species <- factor(c("C. biguttulus",
                           "C. biguttulus",
                           "C. biguttulus",
                           "C. biguttulus",
                           "C. biguttulus",
...
######this is only for looking at specific locations or individuals and not necessary for the thesis
dat.al.t2$loc <- factor(c(  "Reference",
                            "Reference",
                            "Reference",
                            "Reference",
                            "Reference",
...
dat.al.t2$specimen_ID <- factor(c("RP1210",
                                  "RP1211",
                                  "RP1212",
                                  "RP1213",
                                  "RP1214",
...
###NOT RUN: making minimum and maximum PC shapes
...
####here I replaced the number in parantheses with the PC that I wanted to test, e.g. 2:2 = only the second PC)
for (i in 2:2){
  scores <- pca$x[ , i]   
...
df2<-data.frame(pca$x[,1:4])
df2$species<-fourier$species
...
plot(model, points, col=as.numeric(test.data$species))
...
lda<-LDA(fourier$coe, fourier$species, retain=0.95)


###cross validation table as heat map 
plot_CV(lda, rm0=FALSE, fill=TRUE)
...
#create the data frame for visualization of LDA, add species as factor


dfs<-data.frame(lda$mod.pred)
dfs$species<-species

###optional: add loadings to the LDA graph, to see how the PCs affect the LDs

LDAloadings_fourier<-data.frame(Variables=rownames(lda$mod$scaling[1:4,]),lda$mod$scaling[1:4,])



#plot the LDA of fourier analysis, based on the first 4 PCs of the PCA(=95% of explained variation)
lda.plot2 <- dfs%>% 
  ...
  scale_color_manual(values = c("C. brunneus" = "#FFD700", "C. biguttulus" = "#800080", "C. mollis mollis" = "#056608", "C. brunneus ab. ticino" = "#B4641E", "C. mollis ignifer" = "#81D170")) +
  scale_fill_manual(values = c("C. brunneus"="#FFD700","C. biguttulus"="#800080","C. mollis mollis"="#056608","C. brunneus ab. ticino"="#B4641E","C. mollis ignifer"="#81D170"))+
  labs(y = "LD2: 12.6%", x = "LD1: 78%") +
stat_ellipse(aes(fill = species, color = species), geom = "polygon", alpha = 0.1, linetype = 1, lwd = 1.2) +
  guides(color = guide_legend(override.aes = list(linetype = c(0, 0, 0, 0, 0)))) +
  theme(legend.text = element_text(face = c(rep("italic", 5), rep("plain", 5)))) +
  theme(legend.background = element_blank())+
geom_segment(data=LDAloadings_fourier, aes(x=0, y=0, xend=(LD1*0.1), 
                                           yend=(LD2*0.1)), arrow=arrow(length=unit(1, "picas")),
             color="black")+
  annotate("text", x=(LDAloadings_fourier$LD1*0.1), y=(LDAloadings_fourier$LD2*0.1), 
           label=LDAloadings_fourier$Variables)+
  theme(legend.text = element_text(face = c(rep("italic", 5), rep("plain", 5))))
