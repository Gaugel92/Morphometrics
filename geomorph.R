...
#winggpa.pars.d<-dist(winggpa.pars, "euclidean", diag=FALSE, upper=FALSE)
#winggpa.MHV<-betadisper(winggpa.pars.d,species)
#permutest(winggpa.MHV)

#3. shapiro test
winggpa.pars.2<-cbind(winggpa$coords)

winggpa.pars.2
sampled_rows <- sample(1:dim(winggpa.pars.2)[1], size = 5000, replace = FALSE)
subset_matrix <- winggpa.pars.2[sampled_rows, , drop = FALSE]
...
####visualization: show loadings in the PCA (optional) and show the PCA plot
#NOT RUN
...
###pca plot
p2<-df1 %>% 
  ggplot(aes(Comp2, Comp3, color = species), alpha=0.8) +
...
ydens <- axis_canvas(p2, axis = "y", coord_flip = TRUE) + 
  geom_density(data = df1, aes(x = Comp2, fill = species, colour = species), alpha = 0.2) +
  coord_flip()+
  scale_color_manual(values = c("C. brunneus"="#FFD700","C. biguttulus"="#800080","C. mollis mollis"="#056608","C. brunneus ab. ticino"="#B4641E","C. mollis ignifer"="#81D170"))+
  scale_fill_manual(values = c("C. brunneus"="#FFD700","C. biguttulus"="#800080","C. mollis mollis"="#056608","C. brunneus ab. ticino"="#B4641E","C. mollis ignifer"="#81D170"))

p2  %>%  insert_yaxis_grob(ydens, grid::unit(1, "in"), position = "right") %>%
  ggdraw()

 

##split data into training and test set, using first 4 PCs(explain 95% of variation)
df2<-data.frame(lm_PCA$x[,1:4])
df2$species<-species
...
#fit the model
model<-lda(species~., data=train.data,  prior = c(0.173,0.297,0.273,0.148,0.109))
...
###Run LDA on all data 
###columns 107-184 are collinear, so R removed them
df2$species
g.lda<-LDA(lm_PCA$x, species, retain=0.95)
df2<-data.frame(g.lda$mod.pred)

#creating an object for loadings
LDAloadings_LM<-data.frame(Variables=rownames(model$scaling),model$scaling)
...
lda.plot <- df2%>% 
  ...
  scale_color_manual(values = c("C. brunneus" = "#FFD700", "C. biguttulus" = "#800080", "C. mollis mollis" = "#056608", "C. brunneus ab. ticino" = "#B4641E", "C. mollis ignifer" = "#81D170")) +
  scale_fill_manual(values = c("C. brunneus"="#FFD700","C. biguttulus"="#800080","C. mollis mollis"="#056608","C. brunneus ab. ticino"="#B4641E","C. mollis ignifer"="#81D170"))+
  labs(y = "LD2: 25.7%", x = "LD1: 59.8%") +
  stat_ellipse(aes(fill = species, color = species), geom = "polygon", alpha = 0.1, linetype = 1, lwd = 1.2) +
  guides(color = guide_legend(override.aes = list(linetype = c(0, 0, 0, 0, 0)))) +
  theme(legend.text = element_text(face = c(rep("italic", 5), rep("plain", 5)))) +
  theme(legend.background = element_blank()) #+
#geom_segment(data=LDAloadings_LM, aes(x=0, y=0, xend=(LD1*0.1), 
 #                                     yend=(LD2*0.1)), arrow=arrow(length=unit(1, "picas")),
  #           color="black")+
  #annotate("text", x=(LDAloadings_LM$LD1*0.1), y=(LDAloadings_LM$LD2*0.1), 
   #        label=LDAloadings_LM$Variables)+
  #theme(legend.text = element_text(face = c(rep("italic", 5), rep("plain", 5))))



####heat map for model accuracy
plot_CV(g.lda, rm0=FALSE, fill=TRUE, freq=TRUE)


g.lda$CV.tab #cross validation table: this is the absolute numbers of identified individuals
g.lda$CV.correct #proportion of correctly classified individuals, this is average of all
g.lda$CV.ce #class error: this is what is shown in heat map
